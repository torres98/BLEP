...
/*bool has_full_row_rank(const MatrixDS* A) {
    ...
    #if ELEM_BIT_SIZE == 4
        NibbleElem* ref = (NibbleElem*) allocate_memory(n * (n / 2));

        for (uint16_t i = 0; i < n; i++) {
            memcpy(ref + (i * (n / 2)), A -> M + i * (m / 2), n / 2);
        }
    #else
        ELEM_TYPE* ref = (ELEM_TYPE*) allocate_memory((ELEM_BIT_SIZE / 8) * n * n);

        for (uint16_t i = 0; i < n; i++) {
            memcpy(ref + (i * n * (ELEM_BIT_SIZE / 8)), A -> M + i * m * (ELEM_BIT_SIZE / 8), n / 2);
        }
    #endif
