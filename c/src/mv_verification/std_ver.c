...
bool verify_signature(const MatrixDS* M, const MatrixDS* v) {
    if (ncolumns(M) != nrows(v)) {
        fprintf(stderr, "Incompatible sizes for signature verification (public key of size %ux%u and signature of size %u)\n", nrows(M), ncolumns(M), nrows(v));
        exit(1);
    }
