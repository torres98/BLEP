...
def generate_random_linear_transformation(nrows, ncols, min_value, max_value, elem_type=int):
    C = np.empty((nrows, ncols), dtype=elem_type)

    fill_matrix_randomly(C, min_value, max_value, elem_type)
...
def offVer(M, k, min_value, max_value, elem_type=int):
    C = generate_random_linear_transformation(k, M.shape[0], min_value, max_value, elem_type)
