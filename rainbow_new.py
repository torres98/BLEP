    ...
    class RainbowClassic:
        ...
        def __init__(self, pk_path):
            ...
            def parse_public_key(pk_path):
                ...
                Q = Matrix([[0]*N for _ in range(n_polynomials)])
            ...
            self.M = parse_public_key(pk_path)

            #setup public key matrix for Mv verification
            for i in range(n_polynomials):
                identity_row = [gf(0)] * self.M.rows
                identity_row[i] = gf(1)
                self.M[i].extend(identity_row)
        ...
        def get_signature_vector(signature_path, message_path):
            ...
            def parse_signature(signature_path):
                ...
                s = Matrix([z[i] * z[j] for i in range(n_variables) for j in range(i, n_variables)])
            ...
            def compute_digest(filename, salt):
                ...
                return Matrix([gf(int(final_digest[i:i+element_size], 16)) for i in range(0, len(final_digest), element_size)])

            #extract signature information
            s, salt = parse_signature(signature_path)
            h = compute_digest(message_path, salt)

            return Matrix(s.M + h.M)

        def verify(self, v):
            return not (self.M * v).any()
