...
int tc_sha384_init(TCSha384State_t s)
{
	...
	s->iv[0] = 0xcbbb9d5dc1059ed8;
	s->iv[1] = 0x629a292a367cd507;
	s->iv[2] = 0x9159015a3070dd17;
	s->iv[3] = 0x152fecd8f70e5939;
	s->iv[4] = 0x67332667ffc00b31;
	s->iv[5] = 0x8eb44a8768581511;
	s->iv[6] = 0xdb0c2e0d64f98fa7;
	s->iv[7] = 0x47b5481dbefa4fa4;
...
{
	/* input sanity check: */
	if (s == (TCSha384State_t) 0 || data == (void *) 0) {
		return TC_CRYPTO_FAIL;
	} else if (datalen == 0) {
		return TC_CRYPTO_SUCCESS;
	}
	...
	while (datalen-- > 0) {
		...
		if (s->leftover_offset >= TC_SHA384_BLOCK_SIZE) {
			compress(s->iv, s->leftover);
			s->leftover_offset = 0;
			s->bits_hashed += (TC_SHA384_BLOCK_SIZE << 3);
		}
	}
...
int tc_sha384_final(uint8_t *digest, TCSha384State_t s)
{
	...
	/* input sanity check: */
	if (digest == (uint8_t *) 0 || s == (TCSha384State_t) 0) {
		return TC_CRYPTO_FAIL;
	}
	 ...
	 *
	 * NOTE: SHA-384 uses 128 bits for the length of the message, but the
	...
	for (i = 0; i < TC_SHA384_STATE_BLOCKS; ++i) {
...
/*
 * Initializing SHA-384 Hash constant words K.
